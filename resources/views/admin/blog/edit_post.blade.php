...
<script type="text/javascript">
   ClassicEditor
    ...
    .then(editor => {
        ...
        const prefilledValue = `<?php echo addslashes($row->description); ?>`;
        editor.setData(prefilledValue);

        // Set the height and width for media elements after setting the data
        editor.ui.view.editable.element.querySelectorAll('.ck-media__wrapper iframe').forEach(iframe => {
            iframe.style.width = '300px';
            iframe.style.height = '150px';
        });

        // Set the prefilled value on change event
        editor.model.document.on('change', () => {
            const updatedValue = editor.getData();
            var strippedHtml = updatedValue.replace(/<[^>]+>/g, '');
            document.getElementById('seo_description').value = strippedHtml;
        });
