    ...
    <div class="container">
            ...
            <div class="category-header-content">
                ...
                <p class="category-description">
                    @if(isset($main_category))
                        Découvrez les articles de la sous-catégorie <strong>{{$category->name}}</strong> dans la catégorie <strong>{{$main_category->name}}</strong>
                    @else
                        Explorez nos articles sur <strong>{{$category->name}}</strong> et restez informé des dernières actualités et analyses
                    @endif
                </p>
                ...
                <div class="category-stats">
                    <div class="category-stat">
                        ...
                        <span class="stat-label">{{ __("lang.site_articles") }}</span>
                    </div>
                    @if(isset($sub_category) && count($sub_category))
                        <div class="category-stat">
                            <span class="stat-number">{{ count($sub_category) }}</span>
                            <span class="stat-label">Sous-catégories</span>
                        </div>
                    @endif
                    <div class="category-stat">
                        <span class="stat-number">{{ \App\Models\Blog::where('status', 1)->count() }}</span>
                        <span class="stat-label">Total {{ __("lang.site_articles") }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if(isset($sub_category) && count($sub_category))
            <div class="subcategory-navigation">
                <div class="subcategory-nav-title">
                ...
                <div class="subcategory-list">
                    @foreach($sub_category as $sub)
                        <a href="{{ url('category/'.$category->slug.'/'.$sub->slug) }}" 
                           class="subcategory-item {{ Request::segment(3) == $sub->slug ? 'active' : '' }}">
                            <span>{{ $sub->name }}</span>
                        </a>
                    @endforeach
                </div>
        ...
        <div class="results-header">
            <div class="results-count">
                @if(isset($result) && count($result))
                    {{ count($result) }} article(s) trouvé(s) dans cette catégorie
                @else
                    {{ __("lang.site_no_articles_in_category") }}
                @endif
            </div>
            <div class="results-actions">
                <span class="sort-label">{{ __("lang.site_sort_by") }}:</span>
                <select class="sort-select" onchange="sortCategory{{ __("lang.site_articles") }}(this.value)">
            ...
            <div class="col-lg-8 col-md-6 content-area">
                ...
                @else
                    <div class="empty-state">
                        ...
                        <h4>{{ __("lang.site_no_articles_available") }}</h4>
                        <p>Il n'y a actuellement aucun article dans cette catégorie. Revenez plus tard pour découvrir de nouveaux contenus.</p>
...
document.addEventListener('DOMContentLoaded', function() {
    // Sort functionality for category articles
    window.sortCategory{{ __("lang.site_articles") }} = function(sortType) {
