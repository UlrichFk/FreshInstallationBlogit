...
.saved-blogs-header-content {
    position: relative;
    z-index: 2;
}

.saved-blogs-header h3 {
...
.saved-blogs-header .saved-description {
...
<div class="saved-blogs-container">
    <div class="container">
        <!-- Saved Blogs Header -->
        <div class="saved-blogs-header">
            <div class="saved-blogs-header-content">
                <h3>Mes Articles Sauvegardés</h3>
                <p class="saved-description">
                    {{ __("lang.site_saved_articles_description") }}
                </p>
                
                <div class="saved-stats">
                    <div class="saved-stat">
                        <span class="stat-number">{{ count($result) }}</span>
                        <span class="stat-label">Articles Sauvegardés</span>
                    </div>
                    <div class="saved-stat">
        ...
        <div class="results-header">
            <div class="results-count">
                @if(isset($result) && count($result))
                    {{ count($result) }} article(s) sauvegardé(s)
                @else
                    {{ __("lang.site_no_saved_articles") }}
                @endif
            </div>
            <div class="results-actions">
                <span class="sort-label">{{ __("lang.site_sort_by") }}:</span>
                <select class="sort-select" onchange="sortSavedArticles(this.value)">
            ...
            <div class="col-lg-8 col-md-6 content-area">
                                    ...
                                    <div class="article-image">
                                        <div class="article-meta">
                                            <span class="bookmark-btn">
                                                @if(Auth::user()!='')
                                                    <a href="javascript:;" onclick="addRemoveBookMarks('{{$row->id}}','saved');" title="{{__('lang.website_save_post')}}">
                ...
                @else
                    <div class="empty-state">
                        ...
                        <h4>{{ __("lang.site_no_saved_articles") }}</h4>
                        <p>{{ __("lang.site_no_saved_articles_message") }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    // Sort functionality for saved articles
    window.sortSavedArticles = function(sortType) {
