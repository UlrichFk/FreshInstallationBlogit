...
class BlogController extends Controller
{
    ...
    **/
    ...
    { 
        ...
        if(setting('one_signal_key')==''){
            ...
            $token = DeviceToken::select('player_id', DB::raw('MAX(id) as max_id,is_notification_enabled'))->whereIn('user_id',$userIds)->groupBy('player_id')->orderBy('max_id', 'DESC')->get();
            }else{
    ...
    */
    public function analytics(Request $request,$id)
    {
        try{
            ...
            $data['comments'] = Comment::where('blog_id',$id)->with('user')->get();
            if(count($data['comments'])){
                foreach($data['comments'] as $comment){
                    $comment->reported_count = ReportComment::where('comment_id',$comment->id)->count();
                }
            }
            
            $data['commentsCount'] = Comment::where('blog_id',$id)->count();
            $data['viewsCount'] = BlogAnalytic::where('type','view')->where('blog_id',$id)->count();
            $data['likesCount'] = BlogAnalytic::where('type','like')->where('blog_id',$id)->count();
            $data['bookmarksCount'] = BlogBookmark::where('blog_id',$id)->count();
            $data['sharesCount'] = BlogAnalytic::where('type','share')->where('blog_id',$id)->count();
    ...
    {
        try{
            $updated = Comment::updateComment($request->all(),$request->input('id'));
            if($updated['status']==true){
                return redirect()->back()->with('success', $updated['message']); 
            }
            else{
    ...
    **/
    ...
    {
        try{
            $updated = Comment::updateColumn($id,$name,$value);
            if($updated['status']==true){
                return redirect()->back()->with('success', $updated['message']); 
            }
            else{
    ...
    **/
    ...
    {
        try{
            $deleted = Comment::deleteCommentRecord($id);            
            if($deleted['status']==true){
                return redirect()->back()->with('success', $deleted['message']); 
            }
            else{
            ...
            returnredirect()->back()->with('error', $ex->getMessage() . ' '. $ex->getLine() . ' '. $ex->getFile()); 
        }
    }
    ...
    **/
    ...
    {   
        ...
        $blog = Blog::select('id', 'type', 'title', 'description', 'source_name', 'source_link', 'voice', 'accent_code', 'video_url', 'is_voting_enable', 'schedule_date', 'created_at', 'updated_at', 'background_image')
        ->where('schedule_date', date('Y-m-d H:i:00'))
        ->where('status', 4)
        ->with('blog_category')
        ->with('blog_sub_category')
        ->get();
        $image = url('uploads/setting/'.setting('app_logo'));
        // echo date('Y-m-d H:i');
        ...
        if(count($blog)){
            foreach($blog as $row){
                ...
                $status = \Helpers::sendNotification($row->title,$row->description,$image,$row,$player_id);
                if ($status === 200) {
