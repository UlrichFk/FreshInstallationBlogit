    ...
    public function store(Request $request)
    {
        try{
            ...
            if(count($player_id)){
                ...
                {
                    $uploadImage = \Helpers::uploadFilesIfFolderNotExists($post['image'],'push-notification/');
                    if($uploadImage['status']==true)
                    {
                        $push_notification['image'] = $uploadImage['file_name'];
                    }
                }
                $id = PushNotification::insertGetId($push_notification);
                $pushData = PushNotification::where('id',$id)->first();
                
                if($pushData && $pushData->image){
                    $image = url('/uploads/push-notification/'.$pushData->image);
                }else if(file_exists(public_path()."/uploads/setting/".setting('app_logo'))) { 
