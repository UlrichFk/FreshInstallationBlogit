...
class Blog extends Model
{
    ...
    **/
     ...
     * Fetch next blog
    **/
    public static function getNextBlog($blog_id,$type,$category_id){
        try {
            $obj = new self;
            $blogIdArr = array();
            if($type=='category'){
                ...
                $blogs = Blog::whereIn('id',$blogIdArr)->where('type','post')->where('status',1)->latest('schedule_date')->with('image')->get();
            }else if($type=='subcategory'){
                if($category_id!=0){
                    $blogCat = BlogCategory::where('category_id',$category_id)->get();
                    if(count($blogCat)){
                ...
                $blogs = $obj->where('type','post')->where('status',1)->latest('schedule_date')->with('image')->get();
            }

            $currentBlogId = $blog_id; // ID of the current blog you have

            // Find the index of the current blog within the $blogs array
            $currentBlogIndex = $blogs->search(function ($blog) use ($currentBlogId) {
                return $blog->id == $currentBlogId;
            });

            $previousBlog = null;
            $nextBlog = null;

            // Check if the current blog was found in the $blogs array
            if ($currentBlogIndex !== false) {
                $totalBlogs = count($blogs);

                // Find the previous blog if it exists
                if ($currentBlogIndex > 0) {
                    $previousBlog = $blogs[$currentBlogIndex - 1];
                }

                // Find the next blog if it exists
                if ($currentBlogIndex < ($totalBlogs - 1)) {
                    $nextBlog = $blogs[$currentBlogIndex + 1];
                }
            }
            $data['previousBlog'] = $previousBlog;
