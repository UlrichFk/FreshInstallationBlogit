...
//     // Call reinitializeTagify for any language selection
//     reinitializeTagify(langCode);
// }

// function reinitializeTagify(langCode) {
//     setTimeout(function() {
//         var formBody = $("#translation_" + langCode);
//         var tagInput = formBody.find('.tags');
//         var tagInput1 = formBody.find('.seo_tag');

//         // Check if the formBody is visible
//         if (!formBody.is(':visible')) {
//             console.log("Form body for language", langCode, "is not visible.");
//             return; // Exit the function if the formBody is not visible
//         }

//         // Check if the tag input elements are found and visible
//         if (tagInput.length === 0 || !tagInput.is(':visible') ||
//             tagInput1.length === 0 || !tagInput1.is(':visible')) {
//             console.log("Tag inputs for language", langCode, "are not visible or not found.");
//             return; // Exit the function if tag inputs are not ready
//         }

//         // Destroy existing Tagify instances if they exist
//         if (tagifyInstances[langCode]) {
//             tagifyInstances[langCode].destroy();
//             delete tagifyInstances[langCode];
//         }
//         if (tagifyInstances1[langCode]) {
//             tagifyInstances1[langCode].destroy();
//             delete tagifyInstances1[langCode];
//         }

//         // Create new Tagify instances
//         tagifyInstances[langCode] = new Tagify(tagInput[0]);
//         tagifyInstances1[langCode] = new Tagify(tagInput1[0]);
//     }, 300); // Delay of 300 milliseconds to allow for elements to be ready
// }
...
function selectLanguage(langCode) {
    ...
    initializeTagifyOnInteraction(langCode);
}

function initializeTagifyOnInteraction(langCode) {
    var formBody = $("#translation_" + langCode);
    var tagInput = formBody.find('.tags');
    var tagInput1 = formBody.find('.seo_tag');

    // Initialize Tagify when the user focuses on the input
    tagInput.on('focus', function() {
        if (!tagifyInstances[langCode]) {
            tagifyInstances[langCode] = new Tagify(this);
        }
    });
    tagInput1.on('focus', function() {
        if (!tagifyInstances1[langCode]) {
            tagifyInstances1[langCode] = new Tagify(this);
        }
    });
}
...
$(document).ready(function() {
    selectLanguage('en'); // Initialize for the default language
});

// Call this function whenever you need to save the current tags
function saveCurrentTags(langCode) {
    if (tagifyInstances[langCode]) {
        var currentTags = tagifyInstances[langCode].getValue();
        // Store currentTags in a way that you can retrieve them later
    }
}
